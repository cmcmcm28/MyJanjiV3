...
export default function DashboardPage() {
  ...
  const { contracts, getAllContractsForUser, getPendingContractsForUser, loadUserContracts } = useContracts()
            ...
            {upcomingDeadlines.map((contract) => {
              ...
              return (
                <motion.div
                  ...
                  className={`bg-surface rounded-xl p-4 card-shadow cursor-pointer border-l-4 ${
                    daysLeft <= 3 ? 'border-red-500' : daysLeft <= 7 ? 'border-amber-500' : 'border-green-500'
                  }`}
                >
